# NLP Bot - Laboratory 1 (Natural Language Processing)
...
3. **/classifier `"text"`** - Predict a label for a new message using a model trained on collected examples from JSON.
...
1. Clone the repository and move to the project folder.
   Run commands from the Lab01 directory
...
4. Download the Polish spaCy language model:

   ```bash
   python -m spacy download pl_core_news_sm
   ```

5. Create `.env` in the same directory as `bot.py`:
